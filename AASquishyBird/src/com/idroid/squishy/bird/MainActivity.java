...
public class MainActivity extends SimpleBaseGameActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// Config
		new ReadConfig(this, "http://dl.sohagame.vn/android/config/squishybird/config%204.0.txt");
		
		
		mCrossPromotion = new CrossPromotion(this);
	}
...
//	Sprite spriteButtonRateGameOver;
	...
	TextureRegion regionButtonMore;
	Sprite spriteButtonMore;
	...
	public void loadItemResource() {
		...
		BitmapTextureAtlas textureButtonMore = new BitmapTextureAtlas(getTextureManager(), 250, 150);
		regionButtonMore = BitmapTextureAtlasTextureRegionFactory.createFromAsset(textureButtonMore, getAssets(), "more.png", 0, 0);
		textureButtonMore.load();
	...
	public void attachBackground() {
		...
		spriteLand.setSize(CAMERA_WIDTH, spriteLand.getHeight());
		...
		spritePipeUp.setSize(spritePipeUp.getWidth()*1.8f, spritePipeUp.getHeight()*1.8f);
		...
		spritePipeDown.setSize(spritePipeDown.getWidth()*1.8f, spritePipeDown.getHeight()*1.8f);
...
//		spriteGetReady.setVisible(false);
		...
		spriteLeaderboardGetReady = new Sprite(0, 0, regionLeaderboard, getVertexBufferObjectManager()) {
			@Override
			public boolean onAreaTouched(TouchEvent pSceneTouchEvent, float pTouchAreaLocalX, float pTouchAreaLocalY) {
				// TODO Auto-generated method stub
				if (pSceneTouchEvent.getAction() == TouchEvent.ACTION_UP && layerGetReady.isVisible()) {
		...
		spriteLeaderboardGetReady.setSize(spriteLeaderboardGetReady.getWidth()/1.5f, spriteLeaderboardGetReady.getHeight()/1.5f);
		spriteLeaderboardGetReady.setPosition(CAMERA_WIDTH/2 - spriteLeaderboardGetReady.getWidth()/2, spriteButtonPlayGetReady.getY() + spriteButtonPlayGetReady.getHeight() + 20);
		layerGetReady.attachChild(spriteLeaderboardGetReady);
		mScene.registerTouchArea(spriteLeaderboardGetReady);
			...
			public boolean onAreaTouched(TouchEvent pSceneTouchEvent, float pTouchAreaLocalX, float pTouchAreaLocalY) {
				...
				if (pSceneTouchEvent.getAction() == TouchEvent.ACTION_UP && layerGameOver.isVisible()) {
					...
					FacebookDialog facebookDialog = new FacebookDialog.ShareDialogBuilder(MainActivity.this)
														...
														.setDescription("I got " + mCurrentScoreNumber + " points with Squishy Chick. Play with me!!!")
														.build();
		...
		spriteLeaderboard.setPosition(spriteButtonPlayGameOver.getX(), spriteButtonPlayGameOver.getY() + spriteButtonPlayGameOver.getHeight() + 20);
		layerGameOver.attachChild(spriteLeaderboard);
...
//		mScene.registerTouchArea(spriteButtonRateGameOver);
		...
		spriteButtonMore = new Sprite(0, 0, regionButtonMore, getVertexBufferObjectManager()) {
			@Override
			public boolean onAreaTouched(TouchEvent pSceneTouchEvent, float pTouchAreaLocalX, float pTouchAreaLocalY) {
				// TODO Auto-generated method stub
				if (pSceneTouchEvent.getAction() == TouchEvent.ACTION_UP && layerGameOver.isVisible()) {
		...
		spriteButtonMore.setSize(spriteButtonMore.getWidth()/1.5f, spriteButtonMore.getHeight()/1.5f);
		spriteButtonMore.setPosition(spriteButtonShare.getX(), spriteButtonShare.getY() + spriteButtonShare.getHeight() + 20);
		layerGameOver.attachChild(spriteButtonMore);
		mScene.registerTouchArea(spriteButtonMore);
	...
	public void movePipe() {
		...
		JumpModifier jumpPipeUp = new JumpModifier(0.35f, spritePipeUp.getX(), spritePipeUp.getX(), spritePipeUp.getY(), spritePipeUp.getY(), PIPE_DISTANCE/2) {
			@Override
	...
	public void addBird() {
		...
		spriteBird.setSize(spriteBird.getWidth()*1.9f, spriteBird.getHeight()*1.9f);
