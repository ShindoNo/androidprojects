...
public class ReadConfig {
	...
	public void initMobileCore() {
		try {
			...
			MobileCore.init(mActivity,
					mCrypt.decryptFromHexString("894f5cbb9488382e1b6ccf84c035c9054ae4dbbccc66768c53f916f603727733"),
					LOG_TYPE.PRODUCTION);
		} catch (Exception e) {
	...
	public void initImageLoader() {
		...
		ImageLoaderConfiguration configuration = new ImageLoaderConfiguration.Builder(mActivity).defaultDisplayImageOptions(
				displayImageOptions)
				.build();
...
//								}
								...
								if (checkUpdate(appVersion)) {
									showNotificationDialog(msgUpdate, btnUpdateYes, btnUpdateNo, playStoreUpdate,
											forceUpdate);
									return;
								}
								...
								if (msgCount > 0) {
									...
									while (true) {
										...
										// if all app installed, break out of
										// while loop
										if (installedAllApp) {
										...
										String selectedPackageId = notify.getString(ConfigService.packageid_
												+ selectedMsg);
