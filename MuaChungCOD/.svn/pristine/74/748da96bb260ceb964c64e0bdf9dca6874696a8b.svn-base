...
public class LoginActivity extends Activity {
	...
	public void login() {
		...
		if (userName.trim().equals("") || password.trim().equals("")) {
			Tools.toast(getApplicationContext(), getString(R.string.please_fill_up_info));
			return;
		}
		...
		new Thread(new Runnable() {
			public void run() {
				...
				try {
					...
					final JSONObject response = (JSONObject) ServiceHelper.downloadJsonData(url, true);
						...
						public void run() {
							...
							if (response == null) {
								if (!Tools.checkInternet(getApplicationContext())) 
									Tools.toast(getApplicationContext(), getString(R.string.no_internet_connection));
								else 
									Tools.toast(getApplicationContext(), getString(R.string.login_error));
