...
public class DialogDashboard {
				...
				if (event.getAction() == MotionEvent.ACTION_UP) {
					...
					OnDateSetListener onDateSetListener = new OnDateSetListener() {
						...
						public void onDateSet(DatePicker view, int year, int monthOfYear, int dayOfMonth) {
							...
							((EditText) mDialog.findViewById(R.id.et_birthday)).setText(dayOfMonth + "/"
									+ (monthOfYear + 1) + "/" + year);
							mDialog.findViewById(R.id.tv_update_user_info).setEnabled(true);
							((TextView) mDialog.findViewById(R.id.tv_update_user_info)).setTextColor(Color
									.parseColor("#d14129"));
						}
					};
					...
					DatePickerDialog datePickerDialog = new DatePickerDialog(mActivity, onDateSetListener, year, month,
							day);
		...
		mDialog.findViewById(R.id.tv_email_report).setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent.putExtra(
						...
						mActivity.getString(R.string.subject)
								+ mActivity.getString(mActivity.getApplicationInfo().labelRes));
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					dataJSON.put("full_name", ((EditText) mDialog.findViewById(R.id.et_fullname)).getText().toString()
							.trim());
					dataJSON.put("card_no", ((EditText) mDialog.findViewById(R.id.et_cardno)).getText().toString()
							.trim());
					dataJSON.put("address", ((EditText) mDialog.findViewById(R.id.et_address)).getText().toString()
							.trim());
					dataJSON.put("birth_day", ((EditText) mDialog.findViewById(R.id.et_birthday)).getText().toString()
							.trim());
	...
	public void saveUserInfo() {
		try {
			...
			userInfoJSON.put("fullname", ((EditText) mDialog.findViewById(R.id.et_fullname)).getText().toString()
					.trim());
	...
	public void displayPaymentHistory() {
		...
		LinearLayout llPaymentHistoryContainer = (LinearLayout) mDialog
				.findViewById(R.id.ll_addmoney_history_list_container);
		...
		try {
			for (int i = 0; i < mDataAddMoneyHistory.length(); i++) {
				...
				((TextView) paymentHistoryView.findViewById(R.id.tv_game))
						.setText(paymentHistory.getString("app_name"));
				((TextView) paymentHistoryView.findViewById(R.id.tv_type)).setText(paymentHistory.getString("type"));
				((TextView) paymentHistoryView.findViewById(R.id.tv_description)).setText(paymentHistory
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					String apiUrl = Utils.createApiUrl(mActivity, NameSpace.COMMAND_UPDATE_INFO_FACEBOOK,
							dataJSON.toString());
					final String response = ServiceHelper.get(apiUrl);
					...
					mActivity.runOnUiThread(new Runnable() {
						...
						public void run() {
							...
							if (Utils.checkResponseError(mActivity, response) == false) {
								// save new username in shared preference as
								// account_fpay
								...
								mDialog.findViewById(R.id.layout_account_and_addmoneyhistory).setVisibility(
										View.VISIBLE);
								mDialog.findViewById(R.id.ll_account_info).setVisibility(View.VISIBLE);
								mDialog.findViewById(R.id.ll_addmoney_history).setVisibility(View.GONE);

								TextView tvTabAccountInfo = (TextView) mDialog.findViewById(R.id.tv_tab_account_info);
								TextView tvTabAddmoneyHistory = (TextView) mDialog
										.findViewById(R.id.tv_tab_addmoney_history);
			...
			if (v.getId() == R.id.tv_menu_home) {
				...
				try {
					...
					Intent intent = new Intent(Intent.ACTION_VIEW,
							Uri.parse("https://www.facebook.com/groups/1542950539325909/"));
					mActivity.startActivity(intent);
				}
	...
	OnItemClickListener mOnClickNewsListener = new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
			...
			mDialog.findViewById(R.id.lv_dashboard).setVisibility(View.GONE);
			mDialog.findViewById(R.id.webview_dashboard).setVisibility(View.VISIBLE);
			((WebView) mDialog.findViewById(R.id.webview_dashboard)).loadDataWithBaseURL(null, newsContent,
